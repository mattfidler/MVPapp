...
`r "\U0001F4A1"` Whether it makes sense to parse locally is __highly dependent__ on the file itself, e.g. in MVP, NONMEM control streams (`.mod/.ctl`) are considered equivalent to text files (it is converted internally to have a `.txt` extension before uploading to ensure compatibility). Therefore, you may consider locally parsing small text files since there is no advantage to be gained by uploading it. An exception to this would be if there are context window limits which prevents lots of text to fit properly within the prompt.  
