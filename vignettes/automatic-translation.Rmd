...
In another second example, if you already have Claude API access, and just want to change the default model, you could try something like:  
...
From the Model Selection drop-down list, choose the option called "Upload File (AI Translation)". Next, all you need to do is to upload a file (or multiple files, we currently supports PDF or text files `.txt/.mod/.ctl`), click the "Send" button, and then wait for the results to be generated, which takes about a minute.  
