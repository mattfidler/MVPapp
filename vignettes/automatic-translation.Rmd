...
For example, if you want to use Moonshot AI's Kimi K2.5 model (i.e. a OpenAI-Compatible provider), one would launch MVP as follows (after configuring the corresponding `OPENAI_COMPATIBLE_API_KEY` field in .Renviron):  

`run_mvp(model_openai_compatible = "kimi-k2.5", api_chat = "https://api.moonshot.ai/v1/chat/completions")`  

In another second example, if you already have Claude API access, and just want to change the model, you could try something like:  
...
+ While the feature is currently limited to translating and compiling into mrgsolve, in principle it can be quite easily adapted to any other language. The `model_lang` argument controls the output language, which is set to `mrgsolve` by default. For testing purposes, `nonmem` and `rxode2` is also available (MVP app does not support executing NONMEM or RxODE2, so retries will be automatically set to 0).     
