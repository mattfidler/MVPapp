...
`r "\U0001F4A1"` Whether it makes sense to parse locally is __highly dependent__ on the file itself, e.g. in MVP, NONMEM control streams or CPP files (`.mod/.ctl/.cpp`) are considered equivalent to text files (it is converted internally to have a `.txt` extension before uploading to ensure compatibility). Therefore, it will be automatically parsed locally since there is no advantage to be gained by uploading it. If there are context window limits which prevents lots of text to fit within the prompt, the user should then save the file(s) as a PDF so it can properly uploaded.  
